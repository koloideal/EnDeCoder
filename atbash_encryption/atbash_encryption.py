...
def encode_atbash_encryption(string: str, save_register: bool = False) -> str:
    ...
    try:
        first_list_of_ords: list = [
            (ord(x) - 96) if 96 < ord(x) < 123 else x for x in string.lower()
        ]

        second_list_of_ords: list = [
            (27 - x) if isinstance(x, int) else x for x in first_list_of_ords
        ]
